...
*Gearbox, CV joints, and wheels in motion*
...
| Drivetrain | Brushed DC motors with custom 2-stage gearboxes |
| Steering | Ackerman geometry, servo-driven |
| Autopilot | SpeedyBee F405 WING running ArduPilot Rover |
| GPS | Matek M10Q with built-in compass |
| Power | Dual 3S LiPo batteries |
| Wheels | Custom 3D-printed, 17cm diameter |
| RC | ExpressLRS |

## Build Log

### April 2026: Drivetrain running, enclosure v1

Both gearboxes are assembled and installed. The universal joints are in, all four wheels are on, and the drivetrain actually drives. Watching it move across the garage floor under its own power for the first time was genuinely satisfying.

The first electronics enclosure is on the rover. It houses the batteries, flight controller, and ESCs in a 3D-printed box that bolts to the frame. There's a power switch on the side, which makes it feel like a real thing. The wire routing is what you'd expect from a first pass: everything is connected but nothing is managed. That's the next round of work.

What's left to sort out before this thing can navigate on its own: tidy up the enclosure and get the wiring under control, mount the RX externally (the ExpressLRS receiver needs line of sight to work reliably), and get the GPS mounted and tested.

![Rover drivetrain and gearboxes installed, overhead view](images/2026-04-drivetrain-complete-overhead.png)
*Drivetrain complete, all four gearboxes installed and driving*

![Rover with first electronics enclosure installed](images/2026-04-electronics-enclosure-v1-on-rover.png)
*First electronics enclosure on the rover*

### January to March 2026: Gearbox design and CV joints
...
### November to December 2025: Frame, steering, and first test drive
...
The steering knuckles are 3D-printed and implement Ackerman geometry. A Zoskay DS3235 servo provides 35kg-cm of torque, which is enough to turn the wheels even when the rover is sitting still on pavement. I went with Ackerman instead of skid-steer because the inside wheel turns sharper than the outside wheel during a turn, which is how actual cars work. Skid-steer would've been simpler mechanically, but it tears up the tires and fights ArduPilot's steering logic. Ackerman plays nice with the GroundSteering servo function and doesn't leave rubber all over my driveway.
...
The first test drive was humbling. The rover couldn't drive over a small power cord in the garage. When I pushed throttle to the max, one of the motors started smoking. That's when I knew I couldn't get away from learning about gears and gearboxes.
...
### September to October 2025: Wheels and motors

Two motors, two ESCs, one dream. The HOBBYWING QUICRUN 1080 G2 ESCs drive 540 40T brushed motors. I went with brushed over brushless because at 3 mph, efficiency doesn't matter. What matters is smooth low-speed control, and brushed motors do that without needing sensored feedback or fancy ESC tuning. They're also cheaper, and I've already spent enough on this project.

Getting power from the motor shafts to the wheels required flexible couplers. Rigid connections would bind up with any misalignment, and there's always misalignment. The bearings are 6000RS with a 10mm bore, and I spent way too long getting the shaft diameter right. 9.98mm fits a 10mm bore smoothly. 10.02mm does not.
